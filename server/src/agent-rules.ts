 ...
 * ─────────────────────────────────────────────────────────────
 * 移植自 app/src/lib/agent.ts（实现 design/04-ai-agent.md 的 R1-R5 决策表）：
...
export const TIRED_KW = /累|疲惫|撑不住|头痛|没睡好|状态差|不想动|精疲力尽|透支/;
export const EMPTY_KW = /没意义|迷茫|不想做|烦躁|拖延|刷手机|提不起劲/;
export const DOUBT_KW = /方向|不适合|怀疑|放弃|要不要继续|走错/;
export const REVIEW_KW = /总结|复盘|最近怎么样|状态如何|回顾/;
...
// ── ruleChat（无 LLM 时的 fallback 回复，移植自前端 chat()）──

export function ruleChat(input: string, context: ChatContext, userId: string): ChatResult {
  const { user, states, tasks, goals, memories, knowledge = [] } = context;
  const text = input.trim();
  const analysis = analyzeState(states, tasks, memories);
  const vision = goals.find((g) => g.scale === 'year' || g.scale === 'vision');

  // 1) 方向怀疑：冻结计划，开放式对话（规则引擎边界）
  ...
  if (TIRED_KW.test(text)) {
    if (analysis.diagnosis === 'recovery') {
      const plan = generateTodayPlan(analysis, goals, tasks, userId);
      return {
  ...
  // 无 LLM 时仍能从知识库回答“之前记下的方法/笔记是什么”这类查询。
  if (/(知识|笔记|方法|经验|攻略|配方|论文|资源|之前.*记|整理过)/.test(text) && knowledge.length > 0) {
    const terms = text.toLowerCase().split(/[^\p{L}\p{N}]+/u).filter((x) => x.length >= 2);
    const ranked = knowledge
      .map((k) => ({ k, score: terms.reduce((n, term) => n + (k.title.toLowerCase().includes(term) || k.content.toLowerCase().includes(term) ? 1 : 0), 0) }))
      .sort((a, b) => b.score - a.score || b.k.createdAt.localeCompare(a.k.createdAt))
      .slice(0, 3)
      .map(({ k }) => `《${k.title}》：${k.content.replace(/\n+/g, ' ').slice(0, 120)}`);
    return {
      reply: `我在知识库里找到这些可复用内容：\n${ranked.join('\n')}\n如果你问的是其中某一条，告诉我标题或关键词，我可以继续展开。`,
    };
  }
...
export function generateInsight(states: DailyState[], userId: string): MemoryEntry | null {
  const list = sorted(states);
