...
export interface AgentOutput {
  reply: string;
  actions: AgentAction[];
  /** 与回复同一份结构化输出产生的周期承诺；由 Organizer 校验并落库。 */
  recurringTasks: RecurringTaskIntent[];
  intent: LifeOSIntent;
  analysis: AgentAnalysis;
}

export interface RecurringTaskIntent {
  title: string;
...
const OUTPUT_CONTRACT = `只输出 JSON，不要解释，不要 markdown 代码块：
{
  ...
  "recurringTasks": [
    {"title": "用户明确承诺的周期事项", "recurrence": "daily|weekly|monthly", "threadTitle": "仅可填写上下文中已有的活跃线程标题，可选"}
  ]
}
actions 和 recurringTasks 都可以是空数组。
actions 约束：set_mode/apply_plan 只在你确认需要调整今日能量档位或重排计划时给出，最多各一个；plan 内容由系统计算，你不要编造任务列表。
周期承诺约束：只有用户明确说“每天/每周/每月要做、固定 routine、持续执行”的事项才写 recurringTasks；不要从你的建议里创造事项。若 reply 说“会按周期出现/我已设为每日待办”，recurringTasks 必须有对应条目；若没有条目，回复不得作这种承诺。
...
export class SuperAgent {
      ...
      switch (type) {
        case 'set_mode': {
          ...
          actions.push({
            ...
            label: label || `切换到${{ high: '高性能', medium: '平衡', low: '省电' }[mode]}模式`,
          });
          break;
        }
        case 'apply_plan': {
          ...
          actions.push({
            ...
            label: label || `应用今日调整（${plan.keptTasks.length + plan.minimalConnections.length} 件保留 / ${plan.deferredTasks.length} 件顺延）`,
          });
          break;
        }
        case 'freeze_plan': {
          seen.add(type);
          actions.push({ type, label: label || '冻结今日计划（不做自动重排）', reason: intent === 'doubt' ? 'direction_doubt' : intent });
          break;
        }
  ...
  async process(input: string, snapshot: ChatSnapshot): Promise<AgentOutput> {
    ...
    let reply = '';
    let lastResult: { reply?: string; actions?: unknown; recurringTasks?: unknown } = {};
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        lastResult = await this.llm.chatJSON<{ reply?: string; actions?: unknown; recurringTasks?: unknown }>(
          messages,
    ...
    return {
      ...
      recurringTasks: this.hydrateRecurringTasks(lastResult.recurringTasks, snapshot),
      intent,
      analysis,
    };
  }
