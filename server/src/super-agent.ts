...
const OUTPUT_CONTRACT = `只输出 JSON，不要解释，不要 markdown 代码块：
{
  ...
  "taskUpdates": [
    {"taskId": "上下文中已有待办的 id", "newThreadTitle": "目标活跃线程标题"}
  ]
}
...
已有待办修改约束：用户明确要求改期、改周期、改归属线程时，必须写 taskUpdates；先根据语义在【待办】中选出确切 taskId，无法确定时不要声称已修改。若 reply 说“已挪到某线程/已修改”，taskUpdates 必须有对应条目。不能只把这种操作记录成一条记忆。修改已有任务和新建任务互斥：同一事项已有待办时，只写 taskUpdates，绝不再写 taskIntents 创建副本。
...
export class SuperAgent {
  ...
  private hydrateTaskUpdates(rawUpdates: unknown, snapshot: ChatSnapshot): TaskUpdateIntent[] {
    ...
    for (const raw of rawUpdates.slice(0, 2)) {
      ...
      if (!taskId || !taskIds.has(taskId) || !activeTitles.has(newThreadTitle)) continue;
      const key = `${taskId}|${newThreadTitle}`;
      if (seen.has(key)) continue;
      seen.add(key);
      updates.push({ taskId, newThreadTitle });
    }
    return updates;
  }
