...
职责边界：记忆/任务/线程/状态等数据沉淀由后台整理流程（Organizer）负责，你只管回复和上述三种动作，不要输出任何 memories 或 add_task。`;
...
export class SuperAgent {
  ...
  async process(input: string, snapshot: ChatSnapshot): Promise<AgentOutput> {
    ...
    let reply = '';
    let lastResult: { reply?: string; actions?: unknown } = {};
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        lastResult = await this.llm.chatJSON<{ reply?: string; actions?: unknown }>(
          messages,
