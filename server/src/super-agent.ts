...
export class SuperAgent {
  ...
  async process(input: string, snapshot: ChatSnapshot): Promise<AgentOutput> {
    ...
    let reply = '';
    let lastResult: { reply?: string; actions?: unknown; taskIntents?: unknown } = {};
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        lastResult = await this.llm.chatJSON<{ reply?: string; actions?: unknown; taskIntents?: unknown }>(
          messages,
