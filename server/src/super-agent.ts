...
export interface TaskUpdateIntent {
  ...
  newRecurrence?: 'daily' | 'weekly' | 'monthly' | 'once';
  newKind?: 'longterm';
}
...
const OUTPUT_CONTRACT = `只输出 JSON，不要解释，不要 markdown 代码块：
{
  ...
  "taskIntents": [
    {"title": "用户已确认要做的事项", "date": "YYYY-MM-DD，可选", "recurrence": "daily|weekly|monthly（明确重复事务才填写）", "kind": "longterm（长期推进项才填写）", "threadTitle": "仅可填写上下文中已有的活跃线程标题，可选"}
  ],
  "taskUpdates": [
    {"taskId": "上下文中已有待办的 id", "newThreadTitle": "目标活跃线程标题，可选", "newRecurrence": "daily|weekly|monthly|once（重复事务）", "newKind": "longterm（长期推进项）"}
  ]
}
...
已有待办修改约束：用户明确要求改期、改周期、改归属线程时，必须写 taskUpdates；先根据语义在【待办】中选出确切 taskId，无法确定时不要声称已修改。每日/每周/每月发生一次的事务使用 newRecurrence；学习、阅读、项目等没有固定到期日的长期推进项使用 newKind=longterm，绝不能伪装成每周任务。若 reply 说“已挪到某线程/已修改”，taskUpdates 必须有对应条目。不能只把这种操作记录成一条记忆。修改已有任务和新建任务互斥：同一事项已有待办时，只写 taskUpdates，绝不再写 taskIntents 创建副本。
...
export class SuperAgent {
  ...
  private buildContextBlock(snapshot: ChatSnapshot, analysis: AgentAnalysis): string {
    ...
    if (openTasks.length > 0) {
      parts.push('【待办】（修改已有待办时必须原样使用 taskId）' + openTasks
        .slice(-30)
        .map((t) => `${t.id}:${t.title}(执行日${t.date})`)
        .join('；'));
    }
  ...
  private hydrateTaskUpdates(rawUpdates: unknown, snapshot: ChatSnapshot): TaskUpdateIntent[] {
    ...
    const taskIds = new Set(snapshot.tasks.filter((task) => task.status === 'todo').map((task) => task.id));
    const activeTitles = new Set(snapshot.threads.filter((thread) => thread.status === 'active').map((thread) => thread.title));
    ...
    for (const raw of rawUpdates.slice(0, 2)) {
      if (!raw || typeof raw !== 'object') continue;
      const item = raw as Record<string, unknown>;
      const taskId = typeof item.taskId === 'string' ? item.taskId.trim().slice(0, 80) : '';
      const newThreadTitle = typeof item.newThreadTitle === 'string' ? item.newThreadTitle.trim() : '';
      const newRecurrence = typeof item.newRecurrence === 'string' ? item.newRecurrence.trim().toLowerCase() : '';
      const newKind = item.newKind === 'longterm' ? 'longterm' : '';
      if (!taskId || !taskIds.has(taskId)) continue;
      if (newThreadTitle && !activeTitles.has(newThreadTitle)) continue;
      if (newRecurrence && !['daily', 'weekly', 'monthly', 'once'].includes(newRecurrence)) continue;
      if (!newThreadTitle && !newRecurrence && !newKind) continue;
      if (newKind && newRecurrence) continue;
      const key = `${taskId}|${newThreadTitle}|${newRecurrence}|${newKind}`;
      if (seen.has(key)) continue;
      seen.add(key);
      updates.push({
        taskId,
        ...(newThreadTitle ? { newThreadTitle } : {}),
        ...(newRecurrence ? { newRecurrence: newRecurrence as TaskUpdateIntent['newRecurrence'] } : {}),
        ...(newKind ? { newKind: 'longterm' } : {}),
      });
    }
    return updates;
  }
